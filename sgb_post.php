...
if($c){
	...
	echo "---------------------\n";
	echo "Masukkan Interval per post (input hanya angka dalam detik) :\n";
	$interval = trim(fgets(STDIN));
	echo "---------------------\n";
	$lines = file('post.txt');
	echo "[SYSTEM] Menjalankan auto post dengan interval {$interval} detik \n";
	echo "---------------------\n";
	$n=1;
	while(true){
